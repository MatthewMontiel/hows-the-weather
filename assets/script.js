...
let getWeather = function(city) {
  let apiURL = "https:https://api.openweathermap.org/data/2.5/weather?q=" + city + "&appid=8981a04d4bc24c1c6cc03d5640d64cf9=imperial";
  fetch(apiURL)
...
let displaygetWeath = function(weatherData) {
  $("main-city-name").text(weatherData.name + " (" + dayjs(weatherInfo.dt *1000).format("MM/DD/YYYY") + ")").append('<img src="https://openweathermap.org/img/wn/${weatherData.weather[0].icon}@2x.png"></img>');
  ...
  fetch("https://api.openweathermap.org/data/2.5/uvi?lat=" + weatherData.coord.lat + "&lon=" + weatherData.coord.lon + "&appid=8981a04d4bc24c1c6cc03d5640d64cf9")
  .then(function(response){
    response.json().then(function(data) {
...
fetch("https://api.openweathermap.org/data/2.5/forecast?q=" + weatherData.name + "&appid=8981a04d4bc24c1c6cc03d5640d64cf9&units=imperial")
  .then(function(response) {
    response.json().then(function(data) {
      $("#five-day").empty();
...
let saveSearchHist = function (city) {
  if(!searchHist.includes(city)){
    ...
    $("#searchHist").append("<a href='#'class='list-group-item list-group-item-action' id='" + city + "'>" + city + "</a>")
  }
...
let loadSearchHistory = function() {
  ...
  for(i = 0 ; i < searchHist.length ;i++) {
    $("#searchHist").append("<a href='#' class='list-group-item list-group-item-action' id='" + searchHist[i] + "'>" + searchHist[i] + "</a>");
  }
};
